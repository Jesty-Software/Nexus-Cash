...
<div class="grid place-items-center w-full h-full">
  <form action="POST" class="grid gap-3 w-[20rem] text-center font-poppins">
    <h1 class="text-[1.25rem]">Register</h1>
    ...
    {/if}
    <input type="password" placeholder="Confirm Password" class="input {form?.errors?.passwordConfirm ? 'input-bordered input-error' : 'input-bordered'} w-full max-w-xs" />
    ...
    <label class="label cursor-pointer">
      <a href="/legal/tos" class="link"><span class="label-text">I accept the Terms of Service</span></a>
      <input type="checkbox" class="checkbox checkbox-success {form?.errors?.terms ? 'border-error' : ''}" />
    </label>
